...
- (void)setTimeBroadcastView
{
    ...
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@年%d月%d日 %@ %@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,weekString,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)]];
    [self addSubview:nowPickerShowTimeLabel];
    
    if (callBack) {
        callBack([NSString stringWithFormat:@"%@-%d-%d %@:%@:%@",[dateString substringWithRange:NSMakeRange(0, 4)],monthInt,dayInt,[dateString substringWithRange:NSMakeRange(8, 2)],[dateString substringWithRange:NSMakeRange(10, 2)],[dateString substringWithRange:NSMakeRange(12, 2)]]);
    }
...
//    UIView *middleSepView = [[UIView alloc] initWithFrame:CGRectMake(0, 60, 278.5, 30)];
...
{
    yearScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(0, 0, 73.0, 190.0 - rowHeight)];
...
{
    monthScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(73.0, 0, 40.5, 190.0 - rowHeight)];
...
{
    dayScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(113.5, 0, 46.0, 190.0 - rowHeight)];
...
{
    hourScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(159.5, 0, 39.0, 190.0 - rowHeight)];
...
{
    minuteScrollView = [[MXSCycleScrollView alloc] initWithFrame:CGRectMake(198.5, 0, 37.0, 190.0 - rowHeight)];
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSString *weekString = [self fromDateToWeek:dateString];
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d年%d月%d日 %@ %02d:%02d",yearInt,monthInt,dayInt,weekString,hourInt,minuteInt];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
...
//        selectTimeIsNotLegalLabel.text = @"";
    ...
    if (callBack) {
//        callBack([selectTimeString substringToIndex:selectTimeString.length-3]);
        callBack(selectTimeString);
    }
}
//通过日期求星期
...
-(void)reloadData
{
    ...
    [hourScrollView reloadData];
    [minuteScrollView reloadData];
}
@end
