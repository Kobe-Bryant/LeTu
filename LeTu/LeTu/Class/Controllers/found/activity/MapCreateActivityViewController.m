...
{
    if ([self.subject isEqualToString:@""] || self.subject==nil) {
        [self messageToast:@"请填写活动主题!"];
        return;
    }else if ([self.startTime isEqualToString:@""] || self.startTime==nil) {
        [self messageToast:@"请选择活动时间!"];
        return;
    }else if ([self.address isEqualToString:@""] || self.address==nil) {
        [self messageToast:@"请填写活动地点"];
        return;
    }else if ([self.count isEqualToString:@""] || self.count==nil) {
        [self messageToast:@"请填写参与活动人数"];
        return;
    }
    self.publishButton.enabled = NO;
    [self createActivity:self.subject latitude:self.latitude longitude:self.longitude address:self.address count:self.count startTime:self.startTime logImage:self.logImage];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.row) {
        case 0:{//主题
            ...
            [self.navigationController pushViewController:boxVC animated:YES];
             //*/
            MapActivityAddressViewController *addressVC = [[MapActivityAddressViewController alloc] init];
 ...
 *  @param logImage  照片
 */
-(void)createActivity:(NSString*)subject latitude:(NSString*)latitude longitude:(NSString*)longitude address:(NSString*)address count:(NSString*)count startTime:(NSString*)startTime logImage:(UIImage*)logImage
{
    NSLog(@"%@",latitude);
    NSLog(@"%@",longitude);
