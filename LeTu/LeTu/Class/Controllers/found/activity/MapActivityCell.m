...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        self.activityImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 116, 115.5)];
...
//        self.activityContent.text = @"法国爵士音乐鬼才三重奏.法国爵士音乐鬼才三重奏.法国爵士音乐鬼才三重奏.";
        ...
        self.signupImageView = [[UIImageView alloc] initWithFrame:CGRectMake(125, 95, 15, 15)];
        self.signupImageView.image = [UIImage imageNamed:@"people_joined"];
        [self addSubview:self.signupImageView];
        
        self.signupLabel = [[UILabel alloc] initWithFrame:CGRectMake(145, 95, 100, 15)];
        self.signupLabel.textColor = [UIColor grayColor];
//        self.signupLabel.text = @"已报名:30人";
        self.signupLabel.font = [UIFont systemFontOfSize:12.0f];
        ...
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0, 115.5, 320, 0.5)];
        label.backgroundColor = RGBCOLOR(232, 232, 232);
...
-(void)setModel:(MapActivityModel *)model
{
    ...
    if ([model.applyCount isEqualToString:@"0"]) {
        self.signupImageView.hidden = YES;
        self.signupLabel.hidden = YES;
    }else{
        self.signupImageView.hidden = NO;
        self.signupLabel.hidden = NO;
        self.signupLabel.text = [NSString stringWithFormat:@"已报名:%@人",model.applyCount];
    }
}
