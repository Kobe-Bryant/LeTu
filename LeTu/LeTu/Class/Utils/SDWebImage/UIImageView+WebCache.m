...
- (void)setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDWebImageCompletedBlock)completedBlock {
    ...
    if ([manager diskImageExistsForURL:url]) {//图像是否已经被缓存
    ...
    if (url) {
        ...
        id <SDWebImageOperation> operation = [SDWebImageManager.sharedManager downloadWithURL:url options:options progress:progressBlock completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
            ...
            dispatch_main_sync_safe(^{
                if (!wself) return;
                if (image) {
//                    wself.image = [image transformWidth:image.size.width*0.5 height:image.size.height*0.5];
//                    wself.image = [image imageByScalingToSize:CGSizeMake(image.size.width*0.5,image.size.height*0.5)];
                    wself.image = image;
