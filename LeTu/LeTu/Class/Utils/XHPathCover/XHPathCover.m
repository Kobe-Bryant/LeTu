...
//        [_bannerImageView setImageWithURL:[NSURL URLWithString:backgroundImageUrlString]];
        [_bannerImageView setImageWithURL:[NSURL URLWithString:backgroundImageUrlString] placeholderImage:_bannerImageView.image];
    }
}
...
- (void)setAvatarUrlString:(NSString *)avatarUrlString {
    if (avatarUrlString) {
        [_avatarButton setImageWithURL:[NSURL URLWithString:avatarUrlString] forState:UIControlStateNormal placeholderImage:PLACEHOLDER];
    }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    if (iOS_7_Above) {
        self.offsetY = scrollView.contentOffset.y-64;
    }else{
...
- (void)_setup {
    ...
    _bannerImageView.image = [UIImage imageNamed:@"friendscircle_others_pic"];
...
//    [_bannerView addSubview:self.bannerImageViewWithImageEffects];
    ...
    _waterDropRefresh = [[XHWaterDropRefresh alloc] initWithFrame:CGRectMake(35, CGRectGetHeight(self.bounds) - waterDropRefreshHeight-15, waterDropRefreshWidth, waterDropRefreshHeight)];
    _waterDropRefresh.refreshCircleImage = [UIImage imageNamed:@"circle"];
    _waterDropRefresh.offsetHeight = 20+15; // 线条的长度
    [self addSubview:self.waterDropRefresh];
    
    CGFloat avatarButtonHeight = 66;
    self.showUserInfoViewOffsetHeight = CGRectGetHeight(self.frame) - waterDropRefreshHeight / 2 - avatarButtonHeight;
    ...
    _avatarButton = [[UIButton alloc] initWithFrame:CGRectMake(15, 8, 60, 60)];
    _avatarButton.layer.masksToBounds=YES;
    _avatarButton.layer.cornerRadius=_avatarButton.frame.size.height/2;
    [_avatarButton.layer setBorderWidth:2];
    [_avatarButton.layer setBorderColor:[[UIColor whiteColor] CGColor]];
    [_avatarButton addTarget:self action:@selector(clickButton:) forControlEvents:UIControlEventTouchUpInside];
    
    
    _userNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(80, 18, 200, 30)];
...
//    _userNameLabel.shadowOffset = CGSizeMake(0, 2);
    ...
    _birthdayLabel = [[UILabel alloc] initWithFrame:CGRectMake(80, 48, 200, 25)];
...
//    _birthdayLabel.shadowOffset = CGSizeMake(0, 2);
    _birthdayLabel.font = [UIFont systemFontOfSize:15.0f];
